...
CSV_PATH = BASE_DIR / "skater_penalties.csv"
...
if "skaterFullName" in filtered_df.columns:
    search = st.sidebar.text_input("Search player", key="pen_search")
    if search:
        filtered_df = filtered_df[
...
if "pen_page_num" not in st.session_state:
    st.session_state.pen_page_num = 1

if "pen_last_filter_count" not in st.session_state:
    st.session_state.pen_last_filter_count = total_rows

if st.session_state.pen_last_filter_count != total_rows:
    st.session_state.pen_page_num = 1
    st.session_state.pen_last_filter_count = total_rows

if st.session_state.pen_page_num > total_pages:
    st.session_state.pen_page_num = total_pages
if st.session_state.pen_page_num < 1:
    st.session_state.pen_page_num = 1

start_idx = (st.session_state.pen_page_num - 1) * page_size
...
with col1:
    if st.button("◀ Back", disabled=st.session_state.pen_page_num <= 1, key="pen_back"):
        st.session_state.pen_page_num -= 1
        st.rerun()

with col2:
    st.markdown(
        f"<div style='text-align:center; padding-top:0.5rem;'>Page {st.session_state.pen_page_num} of {total_pages}</div>",
        unsafe_allow_html=True
    )

with col3:
    if st.button("Next ▶", disabled=st.session_state.pen_page_num >= total_pages, key="pen_next"):
        st.session_state.pen_page_num += 1
        st.rerun()
