...
CSV_PATH = BASE_DIR / "skater_penaltyShots.csv"
...
desired_order = [
    ...
    "penaltyShotAttempts", "penaltyShotsGoals", "penaltyShotsFailed",
    "penaltyShotShootingPct", "lastName"
]
...
st.subheader("Skater Penalty Shots")
...
with filter_col5:
    if has_player:
        search = st.text_input("Search player", key="ps_search")
        if search:
            filtered_df = filtered_df[
...
if "ps_page_num" not in st.session_state:
    st.session_state.ps_page_num = 1

if "ps_last_filter_count" not in st.session_state:
    st.session_state.ps_last_filter_count = total_rows

if st.session_state.ps_last_filter_count != total_rows:
    st.session_state.ps_page_num = 1
    st.session_state.ps_last_filter_count = total_rows

if st.session_state.ps_page_num > total_pages:
    st.session_state.ps_page_num = total_pages
if st.session_state.ps_page_num < 1:
    st.session_state.ps_page_num = 1

start_idx = (st.session_state.ps_page_num - 1) * page_size
...
with col1:
    if st.button("◀ Back", disabled=st.session_state.ps_page_num <= 1, key="ps_back"):
        st.session_state.ps_page_num -= 1
        st.rerun()

with col2:
    st.markdown(
        f"<div style='text-align:center; padding-top:0.5rem;'>Page {st.session_state.ps_page_num} of {total_pages}</div>",
        unsafe_allow_html=True
    )

with col3:
    if st.button("Next ▶", disabled=st.session_state.ps_page_num >= total_pages, key="ps_next"):
        st.session_state.ps_page_num += 1
        st.rerun()
