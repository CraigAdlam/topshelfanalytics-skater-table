...
CSV_PATH = BASE_DIR / "skater_faceoffwins.csv"
...
if "faceoffwins_page_num" not in st.session_state:
    st.session_state.faceoffwins_page_num = 1

if "faceoffwins_last_filter_count" not in st.session_state:
    st.session_state.faceoffwins_last_filter_count = total_rows

if st.session_state.faceoffwins_last_filter_count != total_rows:
    st.session_state.faceoffwins_page_num = 1
    st.session_state.faceoffwins_last_filter_count = total_rows

if st.session_state.faceoffwins_page_num > total_pages:
    st.session_state.faceoffwins_page_num = total_pages
if st.session_state.faceoffwins_page_num < 1:
    st.session_state.faceoffwins_page_num = 1

start_idx = (st.session_state.faceoffwins_page_num - 1) * page_size
...
with col1:
    if st.button("◀ Back", disabled=st.session_state.faceoffwins_page_num <= 1, key="faceoffwins_back"):
        st.session_state.faceoffwins_page_num -= 1
        st.rerun()

with col2:
    st.markdown(
        f"<div style='text-align:center; padding-top:0.5rem;'>Page {st.session_state.faceoffwins_page_num} of {total_pages}</div>",
        unsafe_allow_html=True
    )

with col3:
    if st.button("Next ▶", disabled=st.session_state.faceoffwins_page_num >= total_pages, key="faceoffwins_next"):
        st.session_state.faceoffwins_page_num += 1
        st.rerun()
...
with col1:
    st.download_button(
        ...
        "text/csv",
        key="faceoffwins_download_filtered"
    )
