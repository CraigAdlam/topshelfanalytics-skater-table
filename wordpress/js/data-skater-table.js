document.addEventListener("DOMContentLoaded", function () {
  ...
  .catch(() => {
    lastUpdatedBox.textContent =
      "Skater data updated: Unavailable";
  });
  ...
  let currentDataset = "bios";

  const datedDatasets = [
    "summary", "faceoffpercentages", "faceoffwins", "goalsforagainst", "penalties",
	"penaltykill", "penaltyshots", "percentages", "powerplay", "puckpossessions",
  ...
  fetch("/wp-json/tsa/v1/skater-team-options")
    .then(response => response.json())
    .then(data => {
  ...
  function getCsvEndpoint(full = false) {
    const base = `/wp-json/tsa/v1/skater-${currentDataset}-csv`;
  ...
  function updateDatasetUI() {
    ...
    if (hasDates) {
      ...
      document.getElementById("tsa-range-start-wrap").style.display = "none";
      document.getElementById("tsa-range-end-wrap").style.display = "none";
      table.setData(currentEndpoint);
    }
  }
  ...
  document.getElementById("tsa-position-filter").addEventListener("change", refreshTable);
